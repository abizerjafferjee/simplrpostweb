...
class Admin_model extends CI_Model
{
    ...
    {
        $this->db->select('userId, profilePicURL, name, userName, emailId, contactNumber');
    ...
    {
        $this->db->select('privateAddresses.addressId, shortName, plusCode, latitude, longitude, imageURL, qrCodeURL, emailId, address,(SELECT GROUP_CONCAT(contactNumber) FROM privateAddressContactNumbers WHERE privateAddressContactNumbers.addressId = privateAddresses.addressId) AS contactNumbers FROM privateAddresses');
        $this->db->where('userId', $userId);
        $this->db->where('status', 1);
        $query = $this->db->get();
        return $query->result();
